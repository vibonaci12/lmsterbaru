    ...
    public function index(Request $request)
    {
        ...
        if ($user->role === 'admin') {
            ...
            if ($guru) {
                ...
                $pengumumanAdminTerbaru = \App\Models\Pengumuman::latest('tanggal')->take(3)->get();
            } else {
                $jadwalHariIni = collect();
                $absensiTerbaru = collect();
                $nilaiTerbaru = collect();
                $pengumumanAdminTerbaru = collect();
            }
            
            return view('dashboard.guru', compact('jadwalHariIni', 'absensiTerbaru', 'nilaiTerbaru', 'pengumumanAdminTerbaru'));
        } else {
            // Siswa
            ...
            if ($siswa) {
                $jadwalMingguan = Jadwal::where('kelas_id', $siswa->kelas_id)
                    ...
                    ->orderBy('jam_mulai')
                    ->get();
                $pengumumanKelas = \App\Models\PengumumanKelas::where('kelas_id', $siswa->kelas_id)
                    ->latest('tanggal')->take(3)->get();
                $pengumumanAdmin = \App\Models\Pengumuman::latest('tanggal')->take(3)->get();
                $pengumumanTerbaru = $pengumumanAdmin->concat($pengumumanKelas)->sortByDesc(function ($item) {
                    return $item->tanggal instanceof \Carbon\Carbon ? $item->tanggal : \Carbon\Carbon::parse($item->tanggal);
                })->take(3)->values();
            }
